...
func (m *Manager) newBackendHealthUpdater(listener config.Listener, addr netip.Addr, healthManager *healthcheck.Manager) (BackendHealthProber, error) {
	var (
		backendMap *ebpf.Map
	)
	log := m.log.
	...
	case 128:
		...
		if err := m.objs.V6ListenerMap.Lookup(listenerKey, &backendMap); err != nil {
			return nil, err
		}
		log.Debug("retrieved backend map", "map", backendMap.String())
		return &backendHealthProber[lbV6Backend]{
			...
			backendMap:    backendMap,
			updateNumBackendFunc: func(num int) error {
				return m.objs.V6NumBackends.Put(listenerKey, uint16(num))
			},
			log: log,
