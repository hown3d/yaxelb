...
func (s *ipv6Suite) Test() {
	testcases := []struct {
		...
		{
			name: "http",
			cmd:  []string{"curl", "--connect-timeout", "3s", "-v", "http://[fd00:db81:1::2]"},
		},
		{
			name: "netcat udp",
			cmd:  []string{"nc", "-6vzu", "-w", "1", "fd00:db81:1::2", "8080"},
		},
		{
			name: "netcat on port not bound by listener",
			cmd:  []string{"nc", "-6vzu", "-w", "1", "fd00:db81:1::2", "4200"},
		},
		{
			name:    "netcat with wrong protocol",
			cmd:     []string{"nc", "-6vz", "-w", "1", "fd00:db81:1::2", "4200"},
			wantErr: true,
		},
	}
